...
(use-package helm
  ...
  :functions (exordium--helm-swith-to-buffer-update-sources
              ...
              exordium--helm-grep-ag-maybe-use-symbol-at-point
  ...
  (helm-grep-ag-command
   ;; Like the default value, but favour ag instead of rg - Exordium uses
   ;; `helm-rg' for the latter.  Also suppress colours in ag, and relay on helm
   ;; highlighting, such that it follows theme.
   (cond ((executable-find "ag")
          "ag --line-numbers -S --no-color --nogroup %s -- %s %s")
         ((executable-find "rg")
