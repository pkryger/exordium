...
(use-package symbol-overlay
  ...
  (setq symbol-overlay-ignore-functions
        (append
         ...
         ;; Add ts-modes handling.  Do it here, as it can't be referred to a
         ;; definiens in a definiendum.  The reason being that, when using
         ;; :custom, the unevaluated definiendum is installed for setting when
         ;; the relevant defcustom is evaluated.  The latter happens when
         ;; feature is loaded.  This is nice as it allows to install a custom
         ;; standard value for variable, without a need to set it to the
         ;; standard.  However, at such a point there's no standard value for
         ;; the variable (the definiens).
         (delq
          nil (mapcar
               (lambda (elt)
