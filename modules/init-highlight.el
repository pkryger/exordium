...
(defvar exordium--symbol-overlay-original-keymap nil
  "Original value of `symbol-overlay-map'.
Used to allow for new bindings according to
`exordium-highlight-symbol-map-modifier' setting when evaluating
`use-package' form.")
...
(use-package symbol-overlay
  ...
  :config
  (unless exordium--symbol-overlay-original-keymap
    (setq exordium--symbol-overlay-original-keymap
  ...
  (if-let* ((template (alist-get exordium-highlight-symbol-map-modifier
      ...
      (progn
        (map-keymap (lambda (key definition)
                    ...
                    exordium--symbol-overlay-original-keymap)
        (bind-keys :map map
          ...
          nil (mapcar
               (lambda (elt)
                 (when-let* ((mode (car elt))
                             ...
                             ((not (eq ts-mode mode)))
                             ((fboundp ts-mode))
                             ((not (assq ts-mode symbol-overlay-ignore-functions))))
